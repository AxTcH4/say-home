...
export default function Hero() {
  ...
  ${mounted ? "opacity-100 translate-y-0" : "opacity-0 "}`}
    >
      ...
      <div
        className={`relative z-10 flex flex-col items-center justify-center h-full text-white text-center px-4 transition-all duration-700 delay-150
  ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
      >
        ...
        <button 
          className={`bg-[#2C1A0E] text-white hover:scale-105 hover:bg-transparent hover:border-2 transition rounded-[1px]
  ${mounted ? "opacity-100 scale-100" : "opacity-0 scale-95"} duration-700 delay-300`}
